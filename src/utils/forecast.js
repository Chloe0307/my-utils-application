...
//  et on a changé response en { body } car on sait que dans body est dans l'objet response, on fait directement appel a body. le résultat est le même
    request({ url, json: true}, (error, { body }) => {
        ...
        if(error) {
            ...
            callback(undefined, 'Aujourd\'hui le temps est ' + body.current.weather_descriptions + '. Il fait actuellement ' + body.current.temperature + ' degrés, et le risque de précipitations est de ' + body.current.precip + '%'
